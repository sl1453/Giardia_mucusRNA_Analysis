...
htseqFiles <- list.files(pattern = "^(control|xanthanGum).*htseqCount$")
htseqFiles
...
sampleNames <- sub("_XSLGU.*","",htseqFiles) #this is removing the ending of the files to better represent the sample names
sampleConditions <- ifelse(substr(htseqFiles, 1, 7) == "control", "c", 
                           ifelse(substr(htseqFiles, 1, 8) == "xanthanG", "g", NA)) #to get conditions I'm pulling the first letter, which is either c, m, or x (control, methylCell, or xanthanGum).
...
sampleConditions <- c('Control1', 'Control2', 'Control3', 'Control4','Gum_1','Gum_2','Gum_3','Gum_4') 
...
res_LFC_g_vs_c <- lfcShrink(dds, coef="condition_g_vs_c", type="apeglm")
res_LFC_g_vs_c
...
resOrdered_g_vs_c <- res_g_vs_c[order(res_g_vs_c$pvalue),]
summary(resOrdered_g_vs_c)
?results
...
#Basic MA-plot
plotMA(res_g_vs_c, ylim=c(-3,3), main="MA-Plot for g vs c")
# See lots of shrinkage towards?
plotMA(res_LFC_g_vs_c, ylim=c(-3,3), main="Shrunk apeglm MA-Plot for g vs c")
...
EnhancedVolcano(res_g_vs_c,
                lab = rownames(res_g_vs_c),
                x = 'log2FoldChange',
                y = 'pvalue',
                selectLab = NA,
                #drawConnectors = TRUE,
...
EnhancedVolcano(res_LFC_g_vs_c,
                lab = rownames(res_LFC_g_vs_c),
                x = 'log2FoldChange',
                y = 'pvalue',
                selectLab = NA,
                #drawConnectors = TRUE,
...
sig_res_g_vs_c
# Write out a table of these significant differentially expressed genes
write.csv(dplyr::select(sig_res_g_vs_c, gene, log2FoldChange, pvalue, padj), 
          file="../../output/Gum_vs_C_pvalue.txt", row.names = F)
##################
...
norm_DEGsig_g_vs_c <- dds_counts %>% 
  data.frame() %>%
  rownames_to_column("gene") %>%
  filter(gene %in% sig_res_g_vs_c$gene) %>% 
  column_to_rownames(var = "gene")


## Annotate our heatmap (optional)
colnames(norm_DEGsig_g_vs_c)
...
res_print <- as.data.frame(resOrdered_g_vs_c)
...
write.xlsx(res_merged[keep_cols], 
           file="../../output/c_Gum/SigDESeq2_resultsNorm_g_vs_c.xlsx", rowNames = F)
#######################################################################
#Write SigDESeq2 normalized results in excel
# Merge DESeq2 results with normalized counts
res_g_vs_c_df <- as.data.frame(res_g_vs_c)
res_g_vs_c_df$geneID <- rownames(res_g_vs_c_df)

merged_data <- merge(res_g_vs_c_df, norm_DEGsig_g_vs_c, by.x="geneID", by.y="row.names")

dat <- queryMany(merged_data$geneID, scopes="symbol", fields=c("name"))
merged_data_with_names <- merge(merged_data, dat, by.x="geneID", by.y="query")

keep_cols <- c("geneID", "name", "log2FoldChange", "pvalue", "padj"  ,"controlrep1_", "controlrep2_", "controlrep3_", "controlrep4_", "xanthanGumrep1_","xanthanGumrep2_","xanthanGumrep3_","xanthanGumrep4_")
output_data <- merged_data_with_names[keep_cols]

# Write out a csv with these data
write.xlsx(output_data, 
           file="../../output/c_Gum/SigDESeq2_resultsNorm_g_vs_c.xlsx", rowNames = F)
