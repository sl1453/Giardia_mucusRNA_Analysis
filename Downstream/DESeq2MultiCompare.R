...
htseqFiles <- list.files(pattern = "*htseq")
htseqFiles
...
#Run DESeq
dds <- DESeq(dds)
res_x_vs_c <- results(dds, contrast=c("condition", "x", "c"))
res_m_vs_c <- results(dds, contrast=c("condition", "m", "c"))
res_x_vs_c
res_m_vs_c
...
resOrdered_m_vs_c <- res_LFC_m_vs_c[order(res_LFC_m_vs_c$pvalue),]
...
plotMA(res_LFC_m_vs_c, ylim=c(-3,3), main="Shrunk MA-Plot for m vs c")
...
norm_DEGsig_x_vs_c <- dds_counts %>% 
  data.frame() %>%
  rownames_to_column("gene") %>%
  filter(gene %in% sig_res_x_vs_c$gene) %>% 
  column_to_rownames(var = "gene")


## Annotate our heatmap (optional)
colnames(norm_DEGsig_x_vs_c)
